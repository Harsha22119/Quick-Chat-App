...
export const AuthProvider = ({ children }) => {
    ...
    const checkAuth = async () => {
        try {
            ...
            if (data?.success) {
                ...
                connectSocket(data.user); // moved here (inside success)
            }
        } catch (error) {
    ...
    const login = async (state, credentials) => {
        try {
            ...
            if (data.success) {
                setAuthUser(data.user);
                connectSocket(data.user); // fixed (was data.userData)

                axios.defaults.headers.common["token"] = data.token; // fixed key
                setToken(data.token);
